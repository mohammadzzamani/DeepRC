...
class RegressionPredictor:
    ...
    def selectAdaptationFactors(self, allFactors, groupsOrder, outcomes,  nFactors, factorSelectionType='rfe', pairedFactors='False', sparse = False, report = False, outputName=''):
    ...
    def _multiXtrain(self, X, y, standardize = True, sparse = False, weightedSample = None, factorAdaptation=False, featureSelectionParameters=None, factorAddition=False, 
                     outputName = '', report=False, factors=None, Xtest=None, ytest=None, factorsTest=None):
        """does the actual regression training, first feature selection: can be used by both train and test
        ...
        if hasMultValuesPerItem(self.cvParams[modelName]) and modelName[-2:] != 'cv':
            ...
            if (len(multiX) > 1):
             ...
             if X.shape[1] < 20:
                    hidden_nodes = [8]
                    save_path = self.root_path + outputName + '/models/ControlsOnly'
                    ...
                    regularization_factor = [0.005 ,0.005, 0.005, 0.005 ,0.005] #0.005
             ...
             epochs = 600#700
             learning_rate =[0.001 ,0.001, 0.0002,0.0002, 0.0002]
             decay = True
             decay_step =1
             ...
             stopping_iteration = [10,10,10,5,5] # if the accuracy didnt improve after this many iterations stop
             stddev = [0.1 , 0.1, 0.1, 0.01]
             ...
             try:
                if not FA:
                   ...
                   regressor.initialize(x1_size = multiX[int(len(multiX)/2)-1].shape[1],x2_size=multiX[0].shape[1],x2n_size=multiX[len(multiX) -2].shape[1],xA_size=multiX[len(multiX) -2].shape[1]) 
                 #   #regressor.set_params(**dict((k, v[0] if isinstance(v, list) else v) for k,v in self.cvParams[modelName][0].items()))
    ...
    def _multiXpredict(self, regressor, X, multiScalers = None, multiFSelectors = None, y = None, sparse = False, 
        ...
        if len(multiX) > 1:
            predictions = []
            for i in range(self.start_phase,self.max_phase):
                print('phase %d'%i)
