...
def init_weights(shape,trainable=True, stddev=0.5,name=None,mean=0.):
    ...
    if name is None:
       ...
       weights = tf.Variable(tf.truncated_normal(shape, stddev=stddev, mean=mean,seed=10),trainable=trainable)
       #b = tf.Variable(initializer(shape))
       #b = tf.Variable(1.)
       b = tf.Variable(tf.zeros(1, dtype=tf.float32),trainable=trainable)
    else:
       #weights = tf.Variable(initializer(shape),name='w_'+name) 
       weights = tf.Variable(tf.truncated_normal(shape, stddev=stddev, mean=mean,seed=10),trainable=trainable,name='w_'+name) 
       #b = tf.Variable(initializer(shape), name='b_'+name)
       b = tf.Variable(0.,name='b_'+name)#tf.zeros(1, dtype=tf.float32),trainable=trainable,name='b_'+name)
    return weights, b
...
class ffNN():
    ...
    def __init__(self, hidden_nodes=[[8,2],[32,8]], epochs=3, learning_rate=0.1, saveFrequency=1,
                 ...
                 activation_function='sigmoid', batch_size=1,shuffle=True,optimizer='Adam',stopping_iteration=[10,10,1,1], stddev=[0.5,0.1],max_phase=4,start_phase=1,FA=False,RC=False,combine_model='yhat',attention_size=32, mask_size=100):
    ...
    def forwardprop(self, X, w_in, b_in, keep_prob,activation_func):
        ...
        z_norm = tf.add(z, b_in,name='add_forward')
        #z_norm = tf.layers.batch_normalization(z_norm, training=is_train)
        if activation_func == 'relu':
            h = tf.nn.relu(z_norm)
        elif activation_func == 'sigmoid':
            h = tf.nn.sigmoid(z_norm,name= 'sigmoid_forward')
    ...
    def forward_pass(self, input_x, phase, reg_factor, keep_prob, x_size, y_size, learning_rate, w_in=None ,b_in= None,prefix='',mean=0.,rg_method='L2') :
            ...
            for i in range(len(self.hidden_nodes[phase-1]) - 1):
                ...
                #l2_norm1 = tf.add(tf.nn.l2_loss(b_in), l2_norm1)
            ...
            return yhat, mse, loss, updates, h_out, learning_rate_adam,l_norm, w_out, b_out, w_in
    ...
    def attentionMechanism(self,control_features, topic_features,size_c,size_t , name='',random_mask=None, mask_size=0, keep_prob=0.5):
            ...
            w_in, b_in = init_weights(shape=[size_c,self.attention_size,mask_size], stddev=1.,name='attention_'+name,mean=0.)
            #w_in = tf.Print(w_in,[w_in, b_in],message = 'w , b ', summarize= 20)
            exps = self.forwardprop( X=control_features, w_in=tf.reshape(w_in,[size_c,-1]), b_in=b_in, keep_prob=keep_prob,activation_func=self.activation_function[-1])
            exps_reshape = tf.reshape(exps, [-1,  self.attention_size,mask_size])

            exps_normal  =  tf.reduce_sum(exps_reshape, 2)
            exps_expanded = tf.expand_dims(exps_normal,-1) * tf.ones([1, 1,mask_size])#* tf.expand_dims(exps_normal,0)
            alphas = exps_reshape /exps_expanded#  tf.reduce_sum(exps_reshape, 2) #tf.reduce_sum(exps, 1)
            #output_attention = tf.reduce_sum(tf.reshape( alphas,[-1,mask_size,self.attention_size]),1)

            #alphas= tf.Print(alphas,[w_in , exps_reshape,exps_normal,exps_expanded,alphas],message='w ,exp , sum, sum expanded, exps normal' , summarize= 10)

            alphas_reshape = tf.reshape(alphas, [-1,  self.attention_size,mask_size])
            output = tf.multiply(topic_expanded,alphas,name='attention_mult')
            output_attention = tf.reduce_sum(output,-1)
            l2_normAtt=tf.add_n([tf.nn.l2_loss(w_in),tf.nn.l2_loss(b_in)])
            print('output shape, ', output_attention.get_shape())
            return output_attention, l2_normAtt
    ...
    def initialize(self, x1_size=11,x2_size=2000,xA_size=22000,x2n_size=20000, y_size=1):
        ...
        #self.random_mask =tf.placeholder('bool',name='random_mask',shape=(self.attention_size,x2_size))
        ...
        with tf.variable_scope("initialization", reuse=tf.AUTO_REUSE):
            ...
            self.yhat1, self.mse1, self.loss1, self.updates1, self.h_out1, self.learning_rate1_adam,self.l2_norm1, _ ,_,_ =self.forward_pass(input_x=self.X1, phase=1,reg_factor=self.reg_factor2,keep_prob=self.keep_prob,x_size=x1_size,y_size=y_size, learning_rate=self.learning_rate1 ) 
            ...
            if self.FA:
               ...
               self.yhatA, self.mseA, self.lossA, self.updatesA , h_outA, self.learning_rate2_adam,self.l2_normA, _, _, self.w_inA =self.forward_pass(input_x=self.XA,phase=2,reg_factor=self.reg_factor2,keep_prob=self.keep_prob,x_size=xA_size,y_size=y_size, learning_rate=self.learning_rate2, prefix='FA',rg_method='L2')
            ...
            if self.combine_model == 'yhat':
                ...
                if self.FA :
                    outputs+= [self.yhatA]
                self.w_out3 = tf.Variable((np.ones([len(outputs),1]) * np.array([[0.5] for i in range(len(outputs))])).astype(np.float32 ),name='w_out_phase3')
                self.b_out3 = tf.Variable(tf.zeros(1, dtype=tf.float32),name='b_out_phase3')
                self.yhat3 = self.forwardprop(tf.concat(outputs,axis=1), self.w_out3, self.b_out3, self.keep_prob,self.activation_function[2]) 
                ...
                self.yhat3, self.mse3, self.loss3, self.updates3 , h_out3, self.learning_rate3_adam,l2_norm3, self.w_out3, self.b_out3, _=self.forward_pass(input_x=tf.concat([self.h_out1, self.h_out2],axis=1),phase=3,reg_factor=self.reg_factor3,keep_prob=self.keep_prob,x_size=last_layer_nodes,y_size=y_size, learning_rate=self.learning_rate3, mean=0.01 )
    ...
    def train(self, X, y, Xtest, ytest):
            ...
            for epoch in range(self.epochs):
                ...
                '''
                [train_x1,train_x2,train_x2n,train_adaptedx,train_y],[train_x1_dev,train_x2_dev,train_x2n_dev,train_adaptedx_dev,train_y_dev]= self.splitShuffle([X1,X2,X2n,XAdapted,y],devSize=0.2, shuffle=self.shuffle)
                train_y_dev = np.transpose(np.array([train_y_dev],dtype=np.float64))
                '''                


                #l = list(np.ones(len(X2)) )
                #sample = random.sample([i for i in range(len(l))] , self.mask_size) 
                #list_sample = [ l[i] if i in sample else 0 for i in range(len(l))]
                #random_mask = np.array([random.sample(list_sample, len(list_sample)) for i in range(self.attention_size)],dtype=bool) #(attention * 2000)
                ...
                for i in range(0, len(train_x1) - self.batch_size + 1, self.batch_size):
                    ...
                    feed_dict={self.reg_factor1: self.regularization_factor[0],self.reg_factor2: self.regularization_factor[1],self.reg_factor3: self.regularization_factor[2],self.reg_factor4: self.regularization_factor[3],self.reg_factor5: self.regularization_factor[4],self.X1: x1_input, self.X2: x2_input,self.X2n: x2n_input, self.XA: xa_input, self.y: y_input, self.keep_prob: keep_prob,self.learning_rate1:self.learning_rate[0] ,self.learning_rate2:self.learning_rate[1]  , self.learning_rate3:self.learning_rate[2] ,self.learning_rate4:self.learning_rate[2]}#,self.random_mask: random_mask   }

                    if  phase >1 and (self.FA or self.FA ==1) :
                          loss = sess.run([eval('self.updates'+str(phase)),eval('self.loss'+str(phase)),eval('self.learning_rate'+str(phase)+'_adam'),eval('self.mse'+str(phase)),eval('self.yhat'+str(phase))],feed_dict= feed_dict)
                ...
                if (bestLoss - epoch_cost < self.stop_loss and stop) or (epoch_cost < self.minimum_cost):
                    pass_best_epochs += 1
                ...
                #        break
                print("Phase= %d, Epoch = %d, train cost = %.6f, train mse:%.6f,test MSE = %.6f "#, weights: %s , weight_b: %.6f, l2norm3: %.6f"
                      % (phase, epoch + 1, epoch_cost, epoch_MSE,MSETest))#, ', '.join(map(str, weights)), weight_b,l2_norm) )
            num_parameters = sum([np.prod(tvar.get_shape().as_list()) for tvar in tf.trainable_variables()])
